#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
MicroserviceDBMongoManager - Exportación e importación/copia de MongoDB
Requisitos:
    pip install pymongo tqdm
Ejemplos:
  Exportar JSON (interactivo):
    python microservice_db_mongo_manager.py export "mongodb://localhost:27017/"
    python microservice_db_mongo_manager.py export "mongodb://localhost:27017/?authMechanism=MONGODB-X509" --x509 "C:/certs/origen.pem"
  Copiar (interactivo) de un Mongo origen a un Mongo destino:
    python microservice_db_mongo_manager.py copy "mongodb://localhost:27017/" "mongodb://localhost:27017/"
    python microservice_db_mongo_manager.py copy "mongodb://localhost:27017/?authMechanism=MONGODB-X509" "mongodb://localhost:27017/" --source-x509 "C:/certs/origen.pem" --dest-x509 "C:/certs/destino.pem"
Modo interactivo:
  - Se configura la ejecución con flechas ↑/↓ + Enter.
    - Export: carpeta de salida, incluir DBs de sistema, timeout (default 60 min).
    - Copy: incluir DBs de sistema, timeout (default 60 min).
  - Luego se selecciona DB y colecciones con flechas y selección.
Nota: las lecturas se hacen en rangos por _id con reintentos para evitar MaxTimeMSExpired
      en colecciones grandes (útil para MongoDB Atlas con límites por operación).
"""

import argparse
import json
import logging
import os
import sys
from dataclasses import dataclass
from datetime import datetime
from pathlib import Path
from typing import Iterable, List, Optional, Sequence
from urllib.parse import urlparse

from bson import ObjectId, json_util
from pymongo import MongoClient
from pymongo.errors import ConnectionFailure, OperationFailure, ServerSelectionTimeoutError

try:
    from tqdm import tqdm

    HAS_TQDM = True
except ImportError:
    HAS_TQDM = False

DEFAULT_TIMEOUT_MS = 60 * 60 * 1000
DEFAULT_RANGE_SIZE = 5000
MAX_RETRIES = 5


def _isatty() -> bool:
    try:
        return sys.stdin.isatty() and sys.stdout.isatty()
    except Exception:
        return False


class _Key:
    UP = "up"
    DOWN = "down"
    ENTER = "enter"
    SPACE = "space"
    ESC = "esc"
    OTHER = "other"


def _read_key() -> str:
    if os.name == "nt":
        import msvcrt

        ch = msvcrt.getch()
        if ch in (b"\x00", b"\xe0"):
            ch2 = msvcrt.getch()
            if ch2 == b"H":
                return _Key.UP
            if ch2 == b"P":
                return _Key.DOWN
            return _Key.OTHER
        if ch == b"\r":
            return _Key.ENTER
        if ch == b" ":
            return _Key.SPACE
        if ch == b"\x1b":
            return _Key.ESC
        return _Key.OTHER
    else:
        import termios
        import tty

        fd = sys.stdin.fileno()
        old = termios.tcgetattr(fd)
        try:
            tty.setraw(fd)
            ch = sys.stdin.read(1)
            if ch == "\x1b":
                ch2 = sys.stdin.read(1)
                if ch2 == "[":
                    ch3 = sys.stdin.read(1)
                    if ch3 == "A":
                        return _Key.UP
                    if ch3 == "B":
                        return _Key.DOWN
                    return _Key.OTHER
                return _Key.ESC
            if ch in ("\r", "\n"):
                return _Key.ENTER
            if ch == " ":
                return _Key.SPACE
            return _Key.OTHER
        finally:
            termios.tcsetattr(fd, termios.TCSADRAIN, old)


def _clear_screen():
    if os.name == "nt":
        os.system("cls")
    else:
        os.system("clear")


def _render_menu(
    title: str,
    lines: Sequence[str],
    cursor: int,
    multi: bool,
    selected: Optional[Sequence[bool]] = None,
    help_text: Optional[str] = None,
):
    _clear_screen()
    print(title)
    print("-" * max(40, len(title)))
    if help_text:
        print(help_text)
        print("")
    for i, line in enumerate(lines):
        prefix = "  "
        if i == cursor:
            prefix = "> "
        if multi:
            mark = "[ ]"
            if selected and i < len(selected) and selected[i]:
                mark = "[x]"
            print(f"{prefix}{mark} {line}")
        else:
            print(f"{prefix}{line}")


def _select_one(title: str, options: Sequence[str], default_index: int = 0) -> Optional[int]:
    if not options:
        return None
    if not _isatty():
        return default_index

    cursor = max(0, min(default_index, len(options) - 1))
    while True:
        _render_menu(
            title=title,
            lines=options,
            cursor=cursor,
            multi=False,
            help_text="↑/↓ para navegar, Enter para seleccionar, Esc para cancelar",
        )
        key = _read_key()
        if key == _Key.UP:
            cursor = (cursor - 1) % len(options)
        elif key == _Key.DOWN:
            cursor = (cursor + 1) % len(options)
        elif key == _Key.ENTER:
            return cursor
        elif key == _Key.ESC:
            return None


def _select_many(title: str, options: Sequence[str], default_selected: Optional[Sequence[bool]] = None) -> Optional[List[int]]:
    if not options:
        return None
    if not _isatty():
        return list(range(len(options)))

    cursor = 0
    selected = [False] * len(options)
    if default_selected:
        for i, v in enumerate(default_selected[: len(options)]):
            selected[i] = bool(v)

    while True:
        _render_menu(
            title=title,
            lines=options,
            cursor=cursor,
            multi=True,
            selected=selected,
            help_text="↑/↓ navegar, Space marcar/desmarcar, Enter confirmar, Esc cancelar",
        )
        key = _read_key()
        if key == _Key.UP:
            cursor = (cursor - 1) % len(options)
        elif key == _Key.DOWN:
            cursor = (cursor + 1) % len(options)
        elif key == _Key.SPACE:
            selected[cursor] = not selected[cursor]
        elif key == _Key.ENTER:
            idx = [i for i, v in enumerate(selected) if v]
            return idx
        elif key == _Key.ESC:
            return None


def _default_output_dir(prefix: str) -> str:
    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    return f"./{prefix}_{ts}"


def _interactive_run_config(mode: str, include_output_dir: bool = True) -> Optional[dict]:
    if not _isatty():
        return {
            "output_dir": _default_output_dir(f"mongo_{mode.lower()}"),
            "include_system": False,
            "timeout_ms": DEFAULT_TIMEOUT_MS,
        }

    state = {
        "output_dir": _default_output_dir(f"mongo_{mode.lower()}"),
        "include_system": False,
        "timeout_ms": DEFAULT_TIMEOUT_MS,
    }

    items = ["include_system", "timeout_ms", "run", "cancel"]
    if include_output_dir:
        items = ["output_dir"] + items
    cursor = 0

    while True:
        lines = []
        if include_output_dir:
            lines.append(f"Carpeta de salida: {state['output_dir']}")
        lines.extend(
            [
                f"Incluir DBs de sistema (admin/local/config/test): {'Sí' if state['include_system'] else 'No'}",
                f"Timeout conexión (ms): {state['timeout_ms']}",
                "INICIAR",
                "CANCELAR",
            ]
        )
        _render_menu(
            title=f"Configurar ejecución ({mode})",
            lines=lines,
            cursor=cursor,
            multi=False,
            help_text="↑/↓ navegar, Enter cambiar/seleccionar, Esc cancelar",
        )

        key = _read_key()
        if key == _Key.UP:
            cursor = (cursor - 1) % len(items)
            continue
        if key == _Key.DOWN:
            cursor = (cursor + 1) % len(items)
            continue
        if key == _Key.ESC:
            return None
        if key != _Key.ENTER:
            continue

        current = items[cursor]
        if current == "run":
            return state
        if current == "cancel":
            return None
        if current == "include_system":
            state["include_system"] = not state["include_system"]
            continue
        if current == "output_dir" and include_output_dir:
            _clear_screen()
            value = input(f"Carpeta de salida (Enter para dejar: {state['output_dir']}): ").strip()
            if value:
                state["output_dir"] = value
            continue
        if current == "timeout_ms":
            _clear_screen()
            value = input(f"Timeout (ms) (Enter para dejar: {state['timeout_ms']}): ").strip()
            if value:
                try:
                    n = int(value)
                    if n > 0:
                        state["timeout_ms"] = n
                except Exception:
                    pass
            continue


@dataclass
class MongoConnOptions:
    uri: str
    timeout_ms: int = DEFAULT_TIMEOUT_MS
    x509_file: Optional[str] = None


class MicroserviceDBMongoManager:
    SYSTEM_DATABASES = {"admin", "local", "config", "test"}

    def __init__(
        self,
        conn: MongoConnOptions,
        output_dir: str = "./mongo_export",
        include_system: bool = False,
        batch_size: int = 500,
        socket_timeout_ms: Optional[int] = None,
        range_size: int = DEFAULT_RANGE_SIZE,
        max_retries: int = MAX_RETRIES,
    ):
        self.conn = conn
        self.output_dir = Path(output_dir)
        self.include_system = include_system
        self.batch_size = batch_size
        self.socket_timeout_ms = socket_timeout_ms if socket_timeout_ms is not None else conn.timeout_ms
        self.range_size = range_size
        self.max_retries = max_retries
        self.client: Optional[MongoClient] = None

        self.stats = {
            "databases": 0,
            "collections": 0,
            "documents": 0,
            "errors": [],
            "selected": [],
            "start_time": None,
            "end_time": None,
        }

        self.output_dir.mkdir(parents=True, exist_ok=True)
        self._setup_logging()

    def _setup_logging(self):
        log_format = "%(asctime)s - %(levelname)s - %(message)s"
        logging.basicConfig(
            level=logging.INFO,
            format=log_format,
            handlers=[logging.FileHandler(self.output_dir / "microservice_db_mongo_manager.log"), logging.StreamHandler(sys.stdout)],
        )
        self.logger = logging.getLogger(__name__)

    def connect(self) -> bool:
        try:
            self.logger.info("Conectando a MongoDB...")
            self.client = self._create_client(self.conn)
            self.client.admin.command("ping")
            server_info = self.client.server_info()
            self.logger.info(f"✓ Conectado exitosamente a MongoDB {server_info.get('version', 'desconocida')}")
            return True
        except ServerSelectionTimeoutError as e:
            self.logger.error(f"✗ No se pudo conectar al servidor: {e}")
            return False
        except ConnectionFailure as e:
            self.logger.error(f"✗ Fallo de conexión: {e}")
            return False
        except Exception as e:
            self.logger.error(f"✗ Error inesperado al conectar: {e}")
            return False

    def close(self):
        try:
            if self.client:
                self.client.close()
        finally:
            self.client = None

    def _build_client_kwargs(self, conn: MongoConnOptions) -> dict:
        kwargs = {
            "serverSelectionTimeoutMS": conn.timeout_ms,
            "connectTimeoutMS": conn.timeout_ms,
            "socketTimeoutMS": self.socket_timeout_ms,
        }

        if conn.x509_file:
            cert_file = Path(conn.x509_file).expanduser()
            if not cert_file.is_file():
                raise FileNotFoundError(f"No se encontró el archivo X.509: {cert_file}")
            kwargs["tls"] = True
            kwargs["tlsCertificateKeyFile"] = str(cert_file)
            self.logger.info(f"Usando certificado X.509: {cert_file}")

        return kwargs

    def _create_client(self, conn: MongoConnOptions) -> MongoClient:
        return MongoClient(conn.uri, **self._build_client_kwargs(conn))

    def get_databases(self) -> List[str]:
        try:
            databases = self.client.list_database_names() if self.client else []
            if not self.include_system:
                databases = [db for db in databases if db not in self.SYSTEM_DATABASES]
            databases.sort()
            return databases
        except OperationFailure as e:
            self.logger.error(f"No se pudieron listar las bases de datos: {e}")
            parsed = urlparse(self.conn.uri)
            if parsed.path and parsed.path.strip("/"):
                return [parsed.path.strip("/")]
            return []

    def get_collections(self, db_name: str) -> List[str]:
        db = self.client[db_name]
        cols = db.list_collection_names()
        cols.sort()
        return cols

    def _upsert_selected(self, mode: str, db: str, **fields):
        selected = self.stats.get("selected")
        if not isinstance(selected, list):
            selected = []
            self.stats["selected"] = selected

        for item in selected:
            if item.get("mode") == mode and item.get("db") == db:
                item.update(fields)
                return

        record = {"mode": mode, "db": db}
        record.update(fields)
        selected.append(record)

    def _sleep_backoff(self, attempt: int):
        import time

        delay = min(2 ** attempt, 30)
        self.logger.warning(f"  Reintentando en {delay}s (intento {attempt + 1}/{self.max_retries})...")
        time.sleep(delay)

    def _is_timeout_error(self, exc: Exception) -> bool:
        import pymongo.errors as pmerr

        msg = str(exc).lower()
        if isinstance(exc, (pmerr.ExecutionTimeout, pmerr.WTimeoutError)):
            return True
        if "maxtimemsexpired" in msg or "operation exceeded time limit" in msg:
            return True
        if "timeouterror" in msg or "timeout" in msg:
            return True
        if isinstance(exc, (pmerr.NetworkTimeout,)):
            return True
        return False

    def _iterate_with_range_pagination(self, collection, projection=None):
        range_size = self.range_size
        attempt = 0
        last_id = None

        while True:
            query = {}
            if last_id is not None:
                query["_id"] = {"$gt": last_id}

            current_range = None
            batch_attempt = 0
            fetched = 0
            while batch_attempt < self.max_retries:
                try:
                    cursor = (
                        collection.find(query, projection=projection)
                        .sort("_id", 1)
                        .limit(range_size)
                        .batch_size(self.batch_size)
                        .hint([("_id", 1)])
                    )
                    current_range = list(cursor)
                    try:
                        cursor.close()
                    except Exception:
                        pass
                    fetched = len(current_range)
                    break
                except Exception as e:
                    if self._is_timeout_error(e) and batch_attempt < self.max_retries - 1:
                        self.logger.warning(f"  ⚠ Timeout leyendo rango (último _id={last_id}): {e}")
                        batch_attempt += 1
                        self._sleep_backoff(batch_attempt)
                        range_size = max(100, range_size // 2)
                        self.logger.warning(f"  Reduciendo range_size a {range_size} para reintento")
                    else:
                        raise

            if fetched == 0:
                return

            for doc in current_range:
                yield doc
                last_id = doc["_id"]

            if fetched < range_size:
                return

    def _write_collection_json_array(self, cursor: Iterable[dict], output_file: Path, total_docs: Optional[int]) -> int:
        with open(output_file, "w", encoding="utf-8") as f:
            f.write("[\n")
            first = True
            count = 0
            if HAS_TQDM and total_docs is not None and total_docs > 0:
                iterator = tqdm(cursor, total=total_docs, desc=f"  {output_file.stem}", unit="docs", leave=False, ncols=80)
            else:
                iterator = cursor

            for doc in iterator:
                if not first:
                    f.write(",\n")
                f.write(json_util.dumps(doc, ensure_ascii=False))
                first = False
                count += 1
            f.write("\n]\n")
        return count

    def export_collection(self, db_name: str, collection_name: str) -> dict:
        db_dir = self.output_dir / db_name
        db_dir.mkdir(parents=True, exist_ok=True)
        output_file = db_dir / f"{collection_name}.json"

        try:
            db = self.client[db_name]
            collection = db[collection_name]

            try:
                total_docs = collection.estimated_document_count()
            except Exception:
                total_docs = None

            self.logger.info(f"  → Exportando {collection_name} (estimado: {total_docs if total_docs is not None else '?'} documentos)")
            iterator = self._iterate_with_range_pagination(collection)
            doc_count = self._write_collection_json_array(iterator, output_file, total_docs)

            file_size = output_file.stat().st_size
            self.logger.info(
                f"  ✓ {collection_name}: "
                f"{doc_count:,} documentos "
                f"({self._human_readable_size(file_size)})"
            )

            self.stats["documents"] += doc_count
            return {"success": True, "documents": doc_count, "file_size": file_size, "file_path": str(output_file)}
        except Exception as e:
            error_msg = f"Error exportando {db_name}.{collection_name}: {str(e)}"
            self.logger.error(f"  ✗ {error_msg}")
            self.stats["errors"].append(error_msg)
            return {"success": False, "error": str(e)}

    def export_database(self, db_name: str, collections: Optional[Sequence[str]] = None) -> dict:
        self.logger.info(f"\n📦 Exportando base de datos: '{db_name}'")
        try:
            all_collections = self.get_collections(db_name)
            if not all_collections:
                self.logger.info("  (Sin colecciones)")
                return {"success": True, "collections": 0, "documents_estimated": 0}

            if collections:
                selected = [c for c in all_collections if c in set(collections)]
            else:
                selected = all_collections

            if not selected:
                self.logger.info("  (Sin colecciones seleccionadas)")
                return {"success": True, "collections": 0, "documents_estimated": 0}

            results = []
            doc_total = 0
            for coll_name in selected:
                r = self.export_collection(db_name, coll_name)
                results.append(r)
                if r.get("success"):
                    doc_total += int(r.get("documents", 0))

            successful = sum(1 for r in results if r.get("success"))
            self.logger.info(f"✓ Base de datos '{db_name}' completada: {successful}/{len(selected)} colecciones")

            return {"success": successful == len(selected), "collections": len(selected), "documents": doc_total, "results": results}
        except Exception as e:
            error_msg = f"Error exportando base de datos '{db_name}': {str(e)}"
            self.logger.error(f"✗ {error_msg}")
            self.stats["errors"].append(error_msg)
            return {"success": False, "error": str(e)}

    def run_export(self, db_name: Optional[str], collections: Optional[Sequence[str]]) -> dict:
        self.stats["start_time"] = datetime.now()

        self.logger.info("=" * 60)
        self.logger.info("INICIANDO MODO EXPORT (JSON)")
        self.logger.info(f"Directorio de salida: {self.output_dir.absolute()}")
        self.logger.info(f"Incluir DBs de sistema: {self.include_system}")
        self.logger.info("=" * 60)

        if not self.connect():
            return {"success": False, "error": "No se pudo conectar"}

        try:
            databases = self.get_databases()
            if not databases:
                self.logger.warning("No se encontraron bases de datos.")
                return {"success": True, "databases": 0}

            if db_name:
                selected_dbs = [db_name]
            else:
                mode_idx = _select_one(
                    "Selecciona bases de datos a exportar",
                    ["(Todas las bases de datos)", "(Seleccionar algunas)"],
                    default_index=0,
                )
                if mode_idx is None:
                    return {"success": False, "error": "Cancelado"}
                if mode_idx == 0:
                    selected_dbs = databases
                else:
                    indices = _select_many("Selecciona DBs", databases)
                    if indices is None:
                        return {"success": False, "error": "Cancelado"}
                    selected_dbs = [databases[i] for i in indices]

            self.logger.info("")
            self.logger.info(f"DBs seleccionadas ({len(selected_dbs)}): {', '.join(selected_dbs)}")

            export_plan: List[dict] = []

            if collections is None:
                collections_mode = _select_one(
                    "Colecciones a exportar",
                    ["(Todas las colecciones en cada DB)", "(Seleccionar colecciones por DB)"],
                    default_index=0,
                )
                if collections_mode is None:
                    return {"success": False, "error": "Cancelado"}
            else:
                collections_mode = 0

            for db in selected_dbs:
                all_cols = self.get_collections(db)
                if not all_cols:
                    self._upsert_selected("EXPORT", db, collections=[], missing_collections=[])
                    export_plan.append({"db": db, "collections": []})
                    continue

                missing: List[str] = []
                selected_cols: List[str] = []

                if collections is not None:
                    requested = list(collections)
                    all_set = set(all_cols)
                    selected_cols = [c for c in requested if c in all_set]
                    missing = [c for c in requested if c not in all_set]
                elif collections_mode == 0:
                    selected_cols = all_cols
                else:
                    indices = _select_many(f"{db} - Selecciona colecciones", all_cols)
                    if indices is None:
                        return {"success": False, "error": "Cancelado"}
                    selected_cols = [all_cols[i] for i in indices]

                self._upsert_selected("EXPORT", db, collections=selected_cols, missing_collections=missing)
                export_plan.append({"db": db, "collections": selected_cols})

            self.logger.info("")
            for item in export_plan:
                db = item["db"]
                cols = item["collections"]
                self.logger.info(f"📦 {db} - Colecciones seleccionadas ({len(cols)}): {', '.join(cols) if cols else '(ninguna)'}")

            for item in export_plan:
                db = item["db"]
                cols = item["collections"]
                if not cols:
                    self.logger.info(f"\n📦 Exportando base de datos: '{db}'")
                    self.logger.info("  (Sin colecciones seleccionadas)")
                    continue

                result = self.export_database(db, cols)
                if result.get("success"):
                    self.stats["databases"] += 1
                    self.stats["collections"] += result.get("collections", 0)
                    self.stats["documents"] += int(result.get("documents", 0))

            self.stats["end_time"] = datetime.now()
            duration = self.stats["end_time"] - self.stats["start_time"]
            self._generate_summary(duration, mode="EXPORT")

            return {"success": len(self.stats["errors"]) == 0, "stats": self.stats, "duration": str(duration)}
        finally:
            self.close()

    def _insert_batch_with_retry(self, dst_col, batch: list) -> int:
        if not batch:
            return 0
        attempt = 0
        while attempt < self.max_retries:
            try:
                dst_col.insert_many(batch, ordered=False)
                return len(batch)
            except Exception as e:
                if self._is_timeout_error(e) and attempt < self.max_retries - 1:
                    self.logger.warning(f"  ⚠ Timeout insertando lote ({len(batch)} docs): {e}")
                    attempt += 1
                    self._sleep_backoff(attempt)
                else:
                    raise
        return 0

    def copy_collections(
        self,
        source_db: str,
        dest_db: str,
        collections: Sequence[str],
        dest_client: MongoClient,
    ) -> dict:
        src_db = self.client[source_db]
        dst_db = dest_client[dest_db]

        existing = set(dst_db.list_collection_names())
        imported = []
        errors = []

        for col in collections:
            try:
                src_col = src_db[col]
                dst_col = dst_db[col]
                refresh_object_ids = col in existing
                deleted_existing_docs = 0

                if refresh_object_ids:
                    delete_result = dst_col.delete_many({})
                    deleted_existing_docs = int(delete_result.deleted_count)

                try:
                    total_docs = src_col.estimated_document_count()
                except Exception:
                    total_docs = None

                self.logger.info(f"  → Copiando {col} (estimado: {total_docs if total_docs is not None else '?'} documentos)")

                inserted = 0
                object_ids_updated = 0
                batch = []
                iterator = self._iterate_with_range_pagination(src_col)

                for doc in iterator:
                    if refresh_object_ids:
                        doc["_id"] = ObjectId()
                        object_ids_updated += 1
                    batch.append(doc)
                    if len(batch) >= self.batch_size:
                        inserted += self._insert_batch_with_retry(dst_col, batch)
                        batch = []
                if batch:
                    inserted += self._insert_batch_with_retry(dst_col, batch)

                imported.append(
                    {
                        "collection": col,
                        "inserted": inserted,
                        "object_ids_updated": object_ids_updated,
                        "existing_dest_collection": refresh_object_ids,
                        "deleted_existing_docs": deleted_existing_docs,
                    }
                )
            except Exception as e:
                errors.append({"collection": col, "error": str(e)})

        return {"success": len(errors) == 0, "imported": imported, "errors": errors}

    def run_copy(
        self,
        source_db: Optional[str],
        collections: Optional[Sequence[str]],
        dest_conn: MongoConnOptions,
    ) -> dict:
        self.stats["start_time"] = datetime.now()

        self.logger.info("=" * 60)
        self.logger.info("INICIANDO MODO COPY (MongoDB -> MongoDB)")
        self.logger.info(f"Origen: {self.conn.uri}")
        self.logger.info(f"Destino: {dest_conn.uri}")
        self.logger.info("Regla: si la colección existe en destino, se vacía y se recopia con _id nuevos")
        self.logger.info("Regla: la DB destino usa el MISMO nombre que la DB origen")
        self.logger.info("=" * 60)

        if not self.connect():
            return {"success": False, "error": "No se pudo conectar al origen"}

        dest_client = None
        try:
            dest_client = self._create_client(dest_conn)
            dest_client.admin.command("ping")

            src_dbs = self.get_databases()
            if not src_dbs:
                return {"success": False, "error": "No se encontraron DBs en el origen"}

            if source_db:
                selected_dbs = [source_db]
            else:
                mode_idx = _select_one(
                    "Selecciona bases de datos a copiar",
                    ["(Todas las bases de datos)", "(Seleccionar algunas)"],
                    default_index=0,
                )
                if mode_idx is None:
                    return {"success": False, "error": "Cancelado"}
                if mode_idx == 0:
                    selected_dbs = src_dbs
                else:
                    indices = _select_many("Selecciona DBs origen", src_dbs)
                    if indices is None:
                        return {"success": False, "error": "Cancelado"}
                    selected_dbs = [src_dbs[i] for i in indices]

            if not selected_dbs:
                return {"success": False, "error": "No se seleccionaron DBs"}

            self.logger.info("")
            self.logger.info(f"DBs seleccionadas ({len(selected_dbs)}): {', '.join(selected_dbs)}")

            if collections:
                collections_mode = 0
            else:
                collections_mode = _select_one(
                    "Colecciones a copiar",
                    ["(Todas las colecciones en cada DB)", "(Seleccionar colecciones por DB)"],
                    default_index=0,
                )
                if collections_mode is None:
                    return {"success": False, "error": "Cancelado"}

            copy_plan: List[dict] = []
            for db in selected_dbs:
                col_names = self.get_collections(db)
                if not col_names:
                    self._upsert_selected("COPY", db, dest_db=db, collections=[], missing_collections=[])
                    copy_plan.append({"db": db, "collections": [], "missing": []})
                    continue

                if collections:
                    cols = [c for c in col_names if c in set(collections)]
                    missing = [c for c in list(collections) if c not in set(col_names)]
                elif collections_mode == 0:
                    cols = col_names
                    missing = []
                else:
                    indices = _select_many(f"{db} - Selecciona colecciones", col_names)
                    if indices is None:
                        return {"success": False, "error": "Cancelado"}
                    cols = [col_names[i] for i in indices]
                    missing = []

                self._upsert_selected("COPY", db, dest_db=db, collections=cols, missing_collections=missing)
                copy_plan.append({"db": db, "collections": cols, "missing": missing})

            overall_success = True
            overall_imported = []
            overall_errors = []

            self.logger.info("")
            for item in copy_plan:
                db = item["db"]
                cols = item["collections"]
                self.logger.info(f"📦 {db} - Colecciones seleccionadas ({len(cols)}): {', '.join(cols) if cols else '(ninguna)'}")

            for item in copy_plan:
                db = item["db"]
                cols = item["collections"]
                missing = item["missing"]
                if not cols:
                    self.logger.info(f"\n📦 {db}: (Sin colecciones seleccionadas, se omite)")
                    continue

                dest_db_name = db
                self.logger.info(f"\n📦 Copiando DB '{db}' -> '{dest_db_name}' ({len(cols)} colecciones)")
                self.logger.info(f"  Colecciones seleccionadas ({len(cols)}): {', '.join(cols)}")
                if missing:
                    self.logger.info(f"  Colecciones no encontradas y omitidas ({len(missing)}): {', '.join(missing)}")
                result = self.copy_collections(db, dest_db_name, cols, dest_client)

                self.stats["databases"] += 1
                self.stats["collections"] += len(cols)
                inserted_total = sum(int(x.get("inserted", 0)) for x in result.get("imported", []))
                self.stats["documents"] += inserted_total

                self._upsert_selected(
                    "COPY",
                    db,
                    dest_db=dest_db_name,
                    imported=result.get("imported", []),
                    errors=result.get("errors", []),
                )

                if result.get("imported"):
                    overall_imported.append({"db": db, "collections": result["imported"]})
                if result.get("errors"):
                    overall_errors.append({"db": db, "errors": result["errors"]})
                    overall_success = False

                if result.get("imported"):
                    imported_txt = ", ".join(
                        (
                            f"{x['collection']} ({x['inserted']}, eliminados: {x.get('deleted_existing_docs', 0)}, _id nuevos: {x.get('object_ids_updated', 0)})"
                            if x.get("existing_dest_collection")
                            else f"{x['collection']} ({x['inserted']})"
                        )
                        for x in result["imported"]
                    )
                    self.logger.info(f"  Importadas: {imported_txt}")
                if result.get("errors"):
                    for e in result["errors"]:
                        self.logger.error(f"  Error en {e['collection']}: {e['error']}")

            self.stats["end_time"] = datetime.now()
            duration = self.stats["end_time"] - self.stats["start_time"]
            self._generate_summary(duration, mode="COPY")

            return {
                "success": overall_success,
                "imported": overall_imported,
                "errors": overall_errors,
                "stats": self.stats,
                "duration": str(duration),
            }
        except Exception as e:
            return {"success": False, "error": str(e)}
        finally:
            try:
                if dest_client:
                    dest_client.close()
            finally:
                self.close()

    def _generate_summary(self, duration, mode: str):
        summary_file = self.output_dir / "RESUMEN_microservice_db_mongo_manager.txt"
        summary_lines = [
            "=" * 60,
            f"RESUMEN MONGO MANAGER ({mode})",
            "=" * 60,
            f"Fecha inicio: {self.stats['start_time'].strftime('%Y-%m-%d %H:%M:%S') if self.stats['start_time'] else 'N/A'}",
            f"Fecha fin: {self.stats['end_time'].strftime('%Y-%m-%d %H:%M:%S') if self.stats['end_time'] else 'N/A'}",
            f"Duración: {duration}",
            "-" * 60,
            f"Bases de datos procesadas: {self.stats.get('databases', 0)}",
            f"Colecciones procesadas: {self.stats.get('collections', 0)}",
            f"Documentos procesados: {self.stats.get('documents', 0):,}",
            f"Errores: {len(self.stats.get('errors', []))}",
            "=" * 60,
        ]

        if self.stats.get("errors"):
            summary_lines.append("\nERRORES ENCONTRADOS:")
            for i, error in enumerate(self.stats["errors"], 1):
                summary_lines.append(f"{i}. {error}")
        else:
            summary_lines.append("\n✓ Operación completada sin errores.")

        selected = self.stats.get("selected") or []
        if selected:
            summary_lines.append("\nSELECCIÓN:")
            for item in selected:
                m = item.get("mode", mode)
                db = item.get("db", "N/A")
                dest_db = item.get("dest_db")
                cols = item.get("collections") or []
                missing = item.get("missing_collections") or []

                if m == "COPY" and dest_db:
                    summary_lines.append(f"\n- {m} DB: {db} -> {dest_db}")
                else:
                    summary_lines.append(f"\n- {m} DB: {db}")

                summary_lines.append(f"  Colecciones seleccionadas: {len(cols)}")
                for c in cols:
                    summary_lines.append(f"    - {c}")

                if missing:
                    summary_lines.append(f"  Colecciones no encontradas (omitidas): {len(missing)}")
                    for c in missing:
                        summary_lines.append(f"    - {c}")

                imported = item.get("imported") or []
                if imported:
                    summary_lines.append(f"  Importadas: {len(imported)}")
                    for x in imported:
                        if x.get("existing_dest_collection"):
                            summary_lines.append(
                                f"    - {x.get('collection')} ({x.get('inserted', 0)}, eliminados: {x.get('deleted_existing_docs', 0)}, _id nuevos: {x.get('object_ids_updated', 0)})"
                            )
                        else:
                            summary_lines.append(f"    - {x.get('collection')} ({x.get('inserted', 0)})")

                errs = item.get("errors") or []
                if errs:
                    summary_lines.append(f"  Errores en colecciones: {len(errs)}")
                    for e in errs:
                        summary_lines.append(f"    - {e.get('collection')}: {e.get('error')}")

        summary_lines.append(f"\nArchivos/logs en: {self.output_dir.absolute()}")
        summary_lines.append("=" * 60)

        summary_text = "\n".join(summary_lines)
        with open(summary_file, "w", encoding="utf-8") as f:
            f.write(summary_text)
        self.logger.info("\n" + summary_text)

    @staticmethod
    def _human_readable_size(size_bytes: int) -> str:
        for unit in ["B", "KB", "MB", "GB", "TB"]:
            if size_bytes < 1024.0:
                return f"{size_bytes:.1f} {unit}"
            size_bytes /= 1024.0
        return f"{size_bytes:.1f} PB"


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="MicroserviceDBMongoManager - Export JSON o copiar colecciones entre MongoDBs",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )

    sub = parser.add_subparsers(dest="command", required=True)

    p_export = sub.add_parser("export", help="Exportar a JSON")
    p_export.add_argument("uri", help="Connection string de MongoDB (origen)")
    p_export.add_argument(
        "--x509",
        "--source-x509",
        dest="source_x509",
        help="Ruta al archivo de certificado/llave X.509 para la conexión de origen",
    )

    p_copy = sub.add_parser("copy", help="Copiar colecciones a otra DB (si existe, reemplaza documentos con _id nuevos)")
    p_copy.add_argument("source", help="Connection string de MongoDB (origen)")
    p_copy.add_argument("dest", help="Connection string de MongoDB (destino)")
    p_copy.add_argument(
        "--source-x509",
        help="Ruta al archivo de certificado/llave X.509 para la conexión de origen",
    )
    p_copy.add_argument(
        "--dest-x509",
        help="Ruta al archivo de certificado/llave X.509 para la conexión de destino",
    )

    return parser


def main():
    parser = _build_parser()
    args = parser.parse_args()

    if args.command == "export":
        cfg = _interactive_run_config("EXPORT", include_output_dir=True)
        if not cfg:
            sys.exit(1)
        manager = MicroserviceDBMongoManager(
            conn=MongoConnOptions(uri=args.uri, timeout_ms=cfg["timeout_ms"], x509_file=args.source_x509),
            output_dir=cfg["output_dir"],
            include_system=cfg["include_system"],
        )
        result = manager.run_export(db_name=None, collections=None)
        if not result.get("success"):
            sys.exit(1)
        return

    if args.command == "copy":
        cfg = _interactive_run_config("COPY", include_output_dir=False)
        if not cfg:
            sys.exit(1)
        manager = MicroserviceDBMongoManager(
            conn=MongoConnOptions(uri=args.source, timeout_ms=cfg["timeout_ms"], x509_file=args.source_x509),
            output_dir=cfg["output_dir"],
            include_system=cfg["include_system"],
        )
        result = manager.run_copy(
            source_db=None,
            collections=None,
            dest_conn=MongoConnOptions(uri=args.dest, timeout_ms=cfg["timeout_ms"], x509_file=args.dest_x509),
        )
        if not result.get("success"):
            sys.exit(1)
        return


if __name__ == "__main__":
    main()